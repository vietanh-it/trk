...
class acf_field_functions
{
    ...
    */
    ...
    function load_value($value, $post_id, $field)
    {
        //region custom code
        global $post, $wpdb;
        if (in_array($post->post_type, ['product', 'recipe', 'banner'])) {
            switch ($post->post_type) {
                case "product" :
                    $table = $wpdb->prefix . "post_info";

                    if (pll_get_post_language($post_id) == 'en') {
                        $original_id = $post_id;
                    }

                    $post_id = pll_get_post($post_id, 'vi');
                    break;
                case "recipe" :
                    $table = $wpdb->prefix . 'post_related';
                    $related_products = $wpdb->get_results("SELECT * FROM {$table} WHERE thanhphan_id = {$post_id}");
                ...
                case 'banner':
                    // create if not exist
                    $banner_model = \TVA\Models\Banners::init();
                    $banner = $banner_model->getBanner($post_id);

                    if (in_array($field['name'], ['start_date', 'end_date'])) // Get date
                    {
                        if (!empty($banner->{$field['name']})) {
                            $banner->{$field['name']} = date('Y/m/d', strtotime($banner->{$field['name']}));
                        }
                    }

                    return $banner->{$field['name']};
                default:
                    $table = $wpdb->prefix . "post_info";
            }

            $cacheId = 'r_extend_info' . $post_id;
            $result = wp_cache_get($cacheId, "default");
            if (false === $result) {
                $query = "SELECT * FROM {$table} WHERE ID = " . $post_id;
                $result = (array)$wpdb->get_row($query);

                $images = unserialize($result['images']);
                $colors = unserialize($result['colors']);

                for ($i = 1; $i <= 4; $i++) {
                    $result['image' . $i] = valueOrNull($images['image' . $i], '');
                    $result['color' . $i] = valueOrNull($colors['color' . $i], '');
                }

                if (!empty($original_id)) {
                    if (empty(get_option('currency_rate'))) {
                        update_option('currency_rate', '22367');
                    }
                    $currency_rate = floatval(get_option('currency_rate'));
                    $result['price'] = number_format($result['price'] / $currency_rate, 2);
                }

                wp_cache_set($cacheId, $result, "default", CACHETIME);
            }
        ...
        switch ($post->post_type) {
            case 'product':
                ...
                if (pll_get_post_language($post_id) == 'en') {
                    ...
                    $data['price'] = intval($data['price'] * $currency_rate);

                    //Get vietnamese post to update
                    $post_id = pll_get_post($post_id, 'vi');
                }
                ...
                $data['colors'] = serialize($colors);

                $product_ctrl = PostController::init();
                $product_ctrl->savePostInfo($post_id, $data);
                break;
            case 'recipe':
                $table = $wpdb->prefix . 'post_related';
                $wpdb->delete($table, ['thanhphan_id' => $post_id]);

                foreach ($data['related_product'] as $product_id) {

                    $wpdb->insert($table, [
                        'thanhphan_id' => $post_id,
            ...
            case 'beauty':
                ...
                $product_ctrl = PostController::init();
                $product_ctrl->savePostInfo($post_id, []);
                break;
            case 'banner':
                if (in_array($field['name'], ['start_date', 'end_date'])) // Get date
                {
                ...
                // create if not exist
                $banner_model = \TVA\Models\Banners::init();
                $banner_model->getBannerInfo($post_id);

                $wpdb->update($wpdb->prefix . 'banner_info', [
